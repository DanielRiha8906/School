...
# Kuřácí oproti nekuřákům mají o 8.19 větší systolický tlak
...
auticka_df <- data.frame(auticka)

s<- step(lm(auticka$hp ~ auticka$mpg + auticka$disp + auticka$drat + auticka$qsec +as.factor(auticka$cyl) + as.factor(auticka$vs) + as.factor(auticka$am) + as.factor(auticka$gear) + as.factor(auticka$carb), auticka_df))
summary(s)
...
anova(lm(auticka$hp ~ auticka$am))
...
# Convert prog to factor with specified levels and labels
p$prog <- factor(p$prog, levels=1:3, labels=c("General", "Academic", "Vocational"))

# Convert id to factor
p$id <- factor(p$id)

# Check updated structure of p
str(p)
